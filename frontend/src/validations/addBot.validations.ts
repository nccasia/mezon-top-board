...
export const ADD_BOT_SCHEMA = yup.object({
  ...
  installLink: yup.string().required('Install Link is required').transform(value => value === '' ? undefined : value).url('Invalid URL').test('url-length', 'URL is too long', val => val.length <= 2082),
  headline: yup.string().required('Headline is required').min(50, 'Minimum 50 characters').max(510, 'Maximum 510 characters'),
  ...
  supportUrl: yup.string().required('Support URL is required').transform(value => value === '' ? undefined : value).url('Invalid URL').test('url-length', 'URL is too long', val => val.length <= 2082),  
